...
"Required" means the field **must be non-empty inside extra**. "(no allowlist)" means the
gateway does not restrict method codes for that currency; the required fields still apply.
...
| `USD` | (no built-in rules) | (gateway decides) |

## Payout method codes

| Currency | Method codes | Required fields |
|---|---|---|
| `ARS` | (no allowlist) | `accountNo`, `accountType`, `address`, `documentNumber`, `documentType`, `email`, `firstName`, `lastName`, `phone` |
